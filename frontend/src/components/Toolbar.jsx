import { Trash2, Eraser, Undo2, Pencil, Square, Circle, Minus, Triangle, MoveRight, Type } from "lucide-react";
  ...
  return (
    <div className="fixed top-5 left-1/2 -translate-x-1/2 z-50 flex items-center gap-4 bg-slate-900/90 backdrop-blur-xl px-6 py-3 rounded-3xl border border-slate-700 shadow-2xl">
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={undoLastStroke} title="Undo" className="p-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-white transition">
          <Undo2 size={17} />
        </button>
        <button onClick={clearCanvas} title="Clear" className="p-2 rounded-xl bg-red-500 hover:bg-red-600 text-white transition">
          <Trash2 size={17} />
        </button>
      </div>
